...
def plot_all_for_scenario(env: str, cfg: dict, year: int = 2050, combined_mode=False,
    ...
    gs = gridspec.GridSpec(6, 4, figure=combined_fig, hspace=0.2, wspace=0.3)
    if combined_mode:
...
def plot_all_scenarios_combined(scenarios: dict, year: int = 2050, figsize=(20, 30)):
    """创建合并的所有情景图表"""
...
def save_figure_properly(fig, base_path):
    ...
    print(f"Figure object saved: {pkl_path}")
    ...
    json_path = f"{base_path}_info.json"
    with open(json_path, "w") as f:
        json.dump(info, f, indent=2)
    print(f"Figure info saved: {json_path}")
...
# 合并模式
fig = plot_all_scenarios_combined(scenarios, year=2050, figsize=(20, 30))
output_path = os.path.join(out_dir, f"06_all_maps")
save_figure_properly(fig, output_path)
fig.show()