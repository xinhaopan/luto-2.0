...
output_png = '11_GHG.png'
# 调用绘图函数，动态指定图的数量和布局，控制图例显示
plot_Combination_figures(ghg_dict, output_png=output_png, input_names=input_files, plot_func=plot_stacked_bar_and_line,
                         categories=categories, color_list=color_list,
                         point_data='Net emissions',
                         n_rows=3, n_cols=3, font_size=12,
                         x_range=(2010, 2050), y_range=(-800, 200),
                         x_ticks=10, y_ticks=200, x_label='Year', y_label='GHG Emissions (Mt CO2e)',
                         legend_position=(0.5, -0.03), show_legend='last', label_positions=(0.01, 0.99))  # 标签位置