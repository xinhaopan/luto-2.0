...
def plot_line_chart(df, ax, font_size=20):
    ...
    ax.set_ylim(0, 120)
    ax.set_yticklabels(range(0, 121, 40), fontsize=font_size)
...
biodiv_gbf_target_2 = calculate_biodiv_gbf_target_2(input_dir)
# 选择2010到2030年的数据
filtered_data = {year: value / 1e6 for year, value in biodiv_gbf_target_2.items() if 2010 <= year <= 2050}
# 转换为 pandas DataFrame
df = pd.DataFrame(list(filtered_data.items()), columns=['Year', 'Biodiv_GBF_Target'])
df.set_index('Year', inplace=True)
df.rename(columns={'Biodiv_GBF_Target': 'Target'}, inplace=True)
tables_dict = get_value_sum(input_files, csv_name, filter_column_name, value_column_name)
tables_dict = {"Target": df, **tables_dict}

# 假设 tables_dict 和 input_files 已经定义
