...
am_dict = get_value_sum(input_files, 'GHG_emissions_separate_no_ag_reduction',  'Land-use', 'Value (t CO2e)')
categories = list(next(iter(am_dict.values())).columns)
color_list = ['#abcd66', '#005ce6', '#c500ff', '#ff0000', '#267300', '#F2A361', '#E56E51', '#FFB715']
plot_Combination_figures(am_dict, output_png='03_GHG_non_ag.png', input_names=input_files, plot_func=plot_stacked_bar,
                         categories=categories, color_list=color_list,
                         n_rows=3, n_cols=3, font_size=12,
                         x_range=(2010, 2050), y_range=(-300, 0),
                         x_ticks=10, y_ticks=100, x_label='Year', y_label='GHG Emissions (Mt CO2e)',
                         legend_position=(0.5, -0.03), show_legend='last', label_positions=(0.01, 0.95),legend_n_rows=2)  # 标签位置