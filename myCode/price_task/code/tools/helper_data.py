...
def amortize_costs(input_file,
                   file_name="cost_transition_ag2non-ag",
                   rate=0.07,
                   period=config.period,
                   when='begin'):
    """
    成本每 `period` 年在节点年发生一次；把该笔成本按年金在 `period` 个自然年内均摊，
    但“不产生中间年”。即把节点年y到y+period-1这些自然年的摊后金额，全部归集到
    对应的“节点年桶”（y自身或下一个节点年）。

    例如 period=5：
      - 2010发生的一次性成本，摊到 2010..2014，每年的摊后额都归到 2015 这个桶；
      - 2015年的摊后额归到 2015（本身是节点年，落到当前桶）。

    参数
    ----
    input_file : 任意可被 get_path / get_year 解析的输入
    file_name  : 源 .npy 前缀名（不含年份）
    rate       : 年利率
    period     : 节点间隔（年），均摊期也是该 period
    when       : 'begin' 期初摊 或 'end' 期末摊（传给 npf.pmt）
    """
    print(f"Start amortizing costs from {input_file} with period={period} years, rate={rate}, when={when}.")
    path_name = get_path(input_file)
    years = sorted(get_year(path_name))  # 节点年，如 [2010, 2015, 2020, ...]
    if not years:
        raise ValueError("No node years found by get_year(path).")
    start_year, end_year = min(years), max(years)

    # 校验 years 是否按 period 等间隔
    if any((years[i] - years[i-1]) != period for i in range(1, len(years))):
        raise ValueError(f"`years` are not spaced by `period={period}` years: {years}")

    # 读取节点年的一次性成本
    costs = {}
    sample_shape = None
    for i in trange(len(years), desc=f"{input_file} amortized years", unit="year"):
        y = years[i]
        f = os.path.join(path_name, f"out_{y}/data_for_carbon_price/{file_name}_{y}.npy")
        arr = np.load(f)
        costs[y] = arr
    ...
    # 仅以节点年作为桶；不生成中间年
    new_costs_bucket = {y: np.zeros(sample_shape, dtype=float) for y in years}

    # 将每个节点年的成本，按年利率在 period 年内均摊；并把每个自然年的摊后额归集到“节点年桶”
    for y in years:
        pv_values = costs[y]  # 节点年的一次性成本（现值）
        annual_payment = -1 * npf.pmt(rate, period, pv=pv_values, fv=0, when=when)
        ...
        for k in range(period):
            ...
            # 把自然年 ty 归到“其对应的节点年桶”（向上取整到最近的节点年）
            # 公式：bucket = ceil_to_period(ty - start_year) + start_year
            bucket = start_year + ((ty - start_year + period - 1) // period) * period

            # 若越过最后节点年，则全部归到最后一个节点年
            if bucket > end_year:
                bucket = end_year

            # 特例：如果 ty 本身就是节点年，则归到当前节点（即不“上取整”越过自己）
            # 上面的公式在 ty 是节点年时会得到 bucket==ty，本身成立，无需额外处理

            new_costs_bucket[bucket] += annual_payment

    # 保存：仅按节点年输出，无中间年文件
    file_name_with_amortised = file_name.replace("cost", "cost_amortised", 1)
    for y in years:
        out_path = os.path.join(
            path_name, f"out_{y}/data_for_carbon_price/{file_name_with_amortised}_{y}.npy"
        )
        save_data(out_path, new_costs_bucket[y])



def calculate_baseline_costs(input_file, use_parallel=True,output=True):
    print(f"Start jobs {input_file} to calculate cost_revenue.")
    path_name = get_path(input_file)
    ...
    if use_parallel:
        results = Parallel(n_jobs=config.N_JOBS)(
            ...
            for year, prefix in tqdm(tasks, desc=f"Processing tasks {input_file}", unit="task")
        )
    else:
        results = []
        for year, prefix in tqdm(tasks, desc=f"Processing tasks {input_file}", unit="task"):
...
def compute_unit_prices(input_file):
    ...
    for i in trange(len(years), desc=f"{input_file} Processing years", unit="year"):
        year = years[i]
        row_data = {"Year": year}
...
def calculate_cost(input_files):
    ...
    for year in trange(years[0], years[-1] + config.period,config.period):
        row_data = {"Year": year}
...
def calculate_shadow_price(input_file,percentile_num=95,mask_use=True, output=True):
    ...
    for year in trange(years[1], years[-1] + config.period,config.period):
        # 加载每年的 .npy 文件
