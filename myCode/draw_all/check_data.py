...
def transform_result_df(result_df, unit):
    """
    将 result_df 的 Source 列解析为 GHG 和 Bio 信息，并合并为一列，替换原 Source 列。
    ...
    def replace_ghg_bio(value):
        parts = value.split("&")  # 拆分左右两部分
        if len(parts) == 2:
            ghg_part, bio_part = parts[0].strip(), parts[1].strip()  # 去除空格
            ghg_part = ghg_replace_dict.get(ghg_part, ghg_part)  # 替换 GHG
            bio_part = bio_replace_dict.get(bio_part, bio_part)  # 替换 Bio
            return f"{ghg_part} - {bio_part}"  # 用 `-` 连接
        return value  # 如果数据格式不匹配，则原样返回
...
group_dict = {key: df.assign(sum=df.sum(axis=1)) for key, df in food_dict.items()}
substring = "*"
target_column = "*" # next(iter(group_dict.values())).columns[0]
year_filter = 2050

next(iter(group_dict.values())).columns
...
result_result = transform_result_df(global_result, "")
result_result