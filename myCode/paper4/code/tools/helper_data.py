...
def summarize_to_type(
    ...
    base_dir = f'../../../output/{config.TASK_NAME}/paper4'
...
def build_profit_and_cost_nc(
    ...
    """
    out_dir = f'../../../output/{config.TASK_NAME}/paper4/1_draw_data'
...
def _make_prices_nc_impl(
    ...
    saves paper4 = cost / carbon and bio_price = cost / bio.
    """
    out_dir = f'../../../output/{config.TASK_NAME}/paper4/1_draw_data'
    ...
    paper4 = cost_all / carbon_all
    bio_price    = cost_all / bio_all
    paper4.name = 'data'
    bio_price.name    = 'data'

    os.makedirs(out_dir, exist_ok=True)
    save2nc(paper4, os.path.join(out_dir, carbon_out))
    save2nc(bio_price,    os.path.join(out_dir, bio_out))

    return paper4, bio_price


def make_prices_nc(output_names):
    """
    Compute paper4 and bio_price from xr_total_cost / carbon / bio NetCDFs.
...
def make_sol_prices_nc(output_names):
    """
    Compute paper4 and bio_price from xr_total_sol_cost / carbon / bio NetCDFs.
    Selects the requested scenarios, collapses extra dims, saves results.
    """
    return _make_prices_nc_impl(
        output_names,
...
def summarize_netcdf_to_excel(
    ...
    """
    input_dir = f'../../../output/{config.TASK_NAME}/paper4/0_base_data'
...
def create_summary(env_category, years, base_path,env_type, colnames):
    ...
    # 计算指标
    xr_paper4_ave_a = (xr_carbon_cost_a.sum(dim="cell") /
                             xr_carbon.sum(dim="cell"))
    da_price = xr_paper4_ave_a["data"].sortby("Year")
