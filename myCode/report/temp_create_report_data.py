...
def save_report_data(sim):
    ...
    files = files[~files['base_name'].isin(["cost transition",
                                            "cost crosstab",
                                            ])]
    files = files[~files['path'].str.contains("revenue_cost_tif")]
    files = files[~files['path'].str.contains("data_for_carbon_price")]
    ...
    heat_area = transition_df_area.style.background_gradient(cmap='Oranges',
                                                             axis=1,
                                                             subset=pd.IndexSlice[:transition_df_area.index[-2],
                                                                    :transition_df_area.columns[-2]]).format('{:,.0f}')
    ...
    DEMAND_DATA_on_off = DEMAND_DATA_long_filter_year.groupby(['Year', 'Type', 'on_off_land']).sum(
    ...
    DEMAND_DATA_commodity = DEMAND_DATA_long_filter_year.groupby(['Year', 'Type', 'COMMODITY']).sum(
        numeric_only=True).reset_index()
    ...
    DEMAND_DATA_commodity_series['color'] = DEMAND_DATA_commodity_series['name'].apply(
        lambda x: COMMODITIES_ALL_COLORS[x])
    ...
    for idx, on_off_land in enumerate(DEMAND_DATA_long['on_off_land'].unique()):
        ...
        DEMAND_DATA_on_off_commodity_wide['color'] = DEMAND_DATA_on_off_commodity_wide['name'].apply(
            lambda x: COMMODITIES_ALL_COLORS[x])

        DEMAND_DATA_on_off_commodity_wide.to_json(
            f'{SAVE_DIR}/production_4_{idx + 1}_demand_domestic_{on_off_land}_commodity.json', orient='records')
    ...
    for idx, Type in enumerate(DEMAND_DATA_long['Type'].unique()):
        ...
        DEMAND_DATA_commodity_wide['color'] = DEMAND_DATA_commodity_wide['name'].apply(
            lambda x: COMMODITIES_ALL_COLORS[x])
        ...
        DEMAND_DATA_commodity_wide.to_json(f'{SAVE_DIR}/production_5_{idx + 1}_demand_{Type}_commodity.json',
                                           orient='records')

    # Plot_2-5-6: Production (LUTO outputs, Million Tonnes)
    quantity_csv_paths = files.query(
        'category == "quantity" and base_name == "quantity_comparison" and year_types == "single_year"').reset_index(
        drop=True)
    ...
    revenue_am_df = files.query(
        'category == "revenue" and base_name == "revenue_agricultural_management" and year_types != "begin_end_year"').reset_index(
        drop=True)
    ...
    revenue_non_ag_df = files.query(
        'category == "revenue" and base_name == "revenue_non_ag" and year_types != "begin_end_year"').reset_index(
        drop=True)
    ...
    cost_am_df = files.query(
        'category == "cost" and base_name == "cost_agricultural_management" and year_types != "begin_end_year"').reset_index(
        drop=True)
    ...
    cost_non_ag_df = files.query(
        'category == "cost" and base_name == "cost_non_ag" and year_types != "begin_end_year"').reset_index(drop=True)
    ...
    cost_transition_ag2ag_df = files.query(
        'category == "cost" and base_name == "cost_transition_ag2ag" and year_types != "begin_end_year"').reset_index(
        drop=True)
    cost_transition_ag2ag_df = pd.concat([pd.read_csv(path) for path in cost_transition_ag2ag_df['path']],
                                         ignore_index=True)
    ...
    cost_transition_ag2non_ag_df = files.query(
        'category == "cost" and base_name == "cost_transition_ag2non_ag" and year_types != "begin_end_year"').reset_index(
        drop=True)
    cost_transition_ag2non_ag_df = pd.concat([pd.read_csv(path) for path in cost_transition_ag2non_ag_df['path']],
                                             ignore_index=True)
    ...
    cost_transition_non_ag2ag_df = files.query(
        'base_name == "cost_transition_non_ag2_ag" and year_types != "begin_end_year"')
    cost_transition_non_ag2ag_df = pd.concat([pd.read_csv(path) for path in cost_transition_non_ag2ag_df['path']],
                                             ignore_index=True)
    ...
    cost_transition_ag2ag_df['Water Supply'] = cost_transition_ag2ag_df['Water Supply'].replace(
    ...
    cost_transition_ag2non_ag_df['Water supply'] = cost_transition_ag2non_ag_df['Water supply'].replace(
        {'dry': 'Dryland', 'irr': 'Irrigated'})
    keep_cols = ['year', 'value (billion)', 'Cost ($)']
    ...
    cost_transition_non_ag2ag_df['Water supply'] = cost_transition_non_ag2ag_df['Water supply'].replace(
        {'dry': 'Dryland', 'irr': 'Irrigated'})
    keep_cols = ['year', 'value (billion)', 'Cost ($)']
    ...
    for idx, col in enumerate(loop_cols):
        ...
        df_wide['type'] = 'column'
        # save to disk
        df_wide.to_json(f'{SAVE_DIR}/economics_10_transition_non_ag2ag_cost_{idx + 1}_{col}_wide.json',
                        orient='records')
    ...
    Cumsum_emissions_json = [
        {'data': list(map(list, zip(Cumsum_emissions['year'], Cumsum_emissions['Cumulative GHG emissions (Mt)']))),
         'type': 'column'}]
    ...
    GHG_files_wide.loc[-1] = ['Net emissions', list(map(list, zip(Net_emission['year'], Net_emission['GHG_sum_Mt']))),
                              'line']
    ...
    GHG_Ag_emission_total_dry_irr = GHG_emissions_long.groupby(['Year', 'Irrigation']).sum()[
        'Quantity (Mt CO2e)'].reset_index()
    ...
    GHG_Ag_emission_total_GHG_type = GHG_emissions_long.groupby(['Year', 'GHG Category']).sum()[
        'Quantity (Mt CO2e)'].reset_index()
    ...
    GHG_Ag_emission_total_Source = GHG_emissions_long.groupby(['Year', 'Sources']).sum()[
        'Quantity (Mt CO2e)'].reset_index()
    ...
    Non_ag_reduction_source = pd.concat([Non_ag_reduction_source, pd.DataFrame(
        {'Year': list(fill_years), 'Sources': 'Agroforestry', 'Quantity (Mt CO2e)': 0})], axis=0)
    ...
    Ag_man_sequestration_long['Irrigation'] = Ag_man_sequestration_long['Irrigation'].replace(
        {'dry': 'Dryland', 'irr': 'Irrigated'})

    # Plot_4-5-1: GHG reductions by Agricultural managements in total (Mt)
    Ag_man_sequestration_total = Ag_man_sequestration_long.groupby(['Year', 'Sources']).sum()[
        'Quantity (Mt CO2e)'].reset_index()
    ...
    Ag_man_sequestration_crop_lvstk_wide = \
    Ag_man_sequestration_long.groupby(['Year', 'Land use category', 'Land category']).sum()[
        'Quantity (Mt CO2e)'].reset_index()
    Ag_man_sequestration_crop_lvstk_wide['Landuse_land_cat'] = Ag_man_sequestration_crop_lvstk_wide.apply(
        lambda x: (x['Land use category'] + ' - ' + x['Land category'])
    ...
    Ag_man_sequestration_crop_lvstk_wide.to_json(f'{SAVE_DIR}/GHG_11_GHG_ag_man_GHG_crop_lvstk_df_wide_Mt.json',
                                                 orient='records')

    # Plot_4-5-3: GHG reductions by Agricultural managements in subsector (Mt)
    Ag_man_sequestration_dry_irr_total = Ag_man_sequestration_long.groupby(['Year', 'Irrigation']).sum()[
        'Quantity (Mt CO2e)'].reset_index()
    ...
    water_paths_total = files.query(
        'category == "water" and year_types == "single_year" and ~base_name.str.contains("separate")').reset_index(
        drop=True)
    water_paths_separate = files.query(
        'category == "water" and year_types == "single_year" and base_name.str.contains("separate")').reset_index(
        drop=True)
    ...
    water_df_separate_lu_type = water_df_separate.groupby(['year', 'Landuse Type']).sum(numeric_only=True)[
        ['Water Use (ML)']].reset_index()
    ...
    water_df_separate_lu_type.loc[len(water_df_separate_lu_type)] = ['Net Volume', list(
        map(list, zip(water_df_net['year'], water_df_net['Water Use (ML)']))), 'line']
    ...
    bio_paths = files.query(
        'category == "biodiversity" and year_types == "single_year" and base_name == "biodiversity_separate"').reset_index(
        drop=True)
