...
def check_null(df_revise):
    # 重置索引，将第一列作为行索引
    df_revise.set_index(df_revise.columns[0], inplace=True)

    # 第一行和第二行为列名，需合并为单一列名
    # 将第一列设为行索引
    df_revise.set_index(df_revise.columns[0], inplace=True)

    # 将第一行和第二行转换为字符串类型后合并为列名
    df_revise.columns = df_revise.iloc[0].astype(str) + "_" + df_revise.iloc[1].astype(str)

    # 去掉前两行，保留数据
    df_revise = df_revise[2:]

    # 检查空值
    if df_revise.isnull().values.any():
        # 找到所有空值的位置
        null_positions = df_revise[df_revise.isnull().any(axis=1)]
        for index, row in null_positions.iterrows():
            null_columns = row[row.isnull()].index.tolist()
            print(f"行名 {index} 存在以下列的空值：{', '.join(null_columns)}")
...
output = "setting_template_windows_3"  # 输出文件名
...
df_revise1 = df_revise.copy()
check_null(df_revise1)
