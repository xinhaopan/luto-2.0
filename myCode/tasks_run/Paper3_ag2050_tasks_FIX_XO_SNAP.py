...
grid_search = {
    'TASK_NAME': ['20260713_Paper3_aquila_FIX_XO_SNAP'],
    'KEEP_OUTPUTS': [False],
    'QUEUE': ['normalsr'],
    ...
    # Identical to the FIX run except the third element: Crossover 0 instead of -1.
    #
    # Barrier converges to a point in the INTERIOR of the feasible region -- every variable
    # strictly between its bounds, none of them exactly zero. Crossover then walks that point
    # out to a vertex, where an optimal solution provably exists and most variables sit
    # exactly at a bound. LUTO never needs the basis crossover produces (the dvars are read
    # back as plain numbers), but it does benefit from the sparsity: write.py skips layers
    # that are all zero, and an interior solution has no all-zero layers -- every land use
    # holds 1e-9 of every cell. So the trade is solve time against output size, memory, and
    # a tail of phantom land uses. This run measures it.
    'RETRY_PARAMS': [[(0, 2, 0, -1, -1), (0, 1, 0, -1, 0)]],
    # Crossover off AND the interior-point dust snapped to zero at the ROUND_DECIMALS floor.
    # The FIX_XO run turns crossover off but keeps the dust, so comparing the two isolates what
    # the snap actually buys: sparsity, memory, and outputs without phantom land uses.
    'SNAP_DVAR_NOISE_TO_ZERO': [True],
