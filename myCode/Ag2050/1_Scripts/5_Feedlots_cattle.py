...
# 读取原始数据
df = pd.read_excel('../0_original_data/cattle_number.xlsx')
...
# ---------- 预测增长指数并绘图 ----------
ax, df_result = predict_growth_index(df, var_name='Cattle number (head)',base_year = 1996,model='ETS',draw_base_year=1996, use_index=False)
handles, labels = ax.get_legend_handles_labels()
ax.legend(handles, labels, loc='upper left', frameon=True)
ax.set_title('Feedlots cattle number')
ax.set_ylabel('head')

fig = ax.get_figure()
fig.savefig('cattle_number.png', dpi=300)
fig.show()
df_result.to_excel('../2_processed_data/cattle_number_forecast.xlsx', index=True)
