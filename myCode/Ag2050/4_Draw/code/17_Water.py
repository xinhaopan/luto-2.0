...
def _load_climate_impact(scenario):
    ...
    Reads 04_water_climate_impact_split.csv (generated by 04_indicators.py from the raw
    ...
    if _CLIMATE_WATER_CACHE is None:
        path = os.path.join(EXCEL_DIR, '04_water_climate_impact_split.csv')
        if not os.path.exists(path):
            raise FileNotFoundError(
                f'Missing water climate-impact split: {path}\n'
                'Run  python 04_indicators.py  (xpluto env) first — it generates this cache '
...
def prepare_overview():
    ...
    the 04_water_climate_impact_split.csv cache generated by 04_indicators.py). Land use
...
def main():
    workbook = '17_water_long_tables.xlsx'
    if GENERATE_TABLES:
        export_long_tables(
            workbook,
