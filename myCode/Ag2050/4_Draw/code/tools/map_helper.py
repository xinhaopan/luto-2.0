...
def _load_color_mapping(colors_sheet):
    data = pd.read_excel('tools/land use colors.xlsx', sheet_name=colors_sheet).copy()

    if 'desc_new' in data.columns:
        data['desc'] = data['desc_new'].fillna(data['desc'])

    if colors_sheet == 'ag_group_map':
        overrides = {
            'crop': '#AECB75',
            'nonag': '#3A7F4A',
        }
        normalized_desc = data['desc'].map(_normalize_label)
        for key, color in overrides.items():
            data.loc[normalized_desc == key, 'color'] = color
...
def assign_colors_to_overlay(overlay_reproject_geo_tif, colors_sheet, overlay_reproject_rgb_tif, background_value=-9999,write_png=False):
    ...
    # 读取颜色映射
    color_mapping = _load_color_mapping(colors_sheet)
...
def create_legend(colors_sheet, legend_png_name = "legend_png.png", legend_title="Legend", legend_fontsize=8,
    ...
    # 读取 CSV 文件
    data = _load_color_mapping(colors_sheet)
    if colors_sheet == 'am':
        data = data[data['desc'] != AM_LEGEND_EXCLUDE].copy()
