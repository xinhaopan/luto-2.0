...
import os as _os
_CONFIG_DIR  = _os.path.dirname(_os.path.abspath(__file__))   # tools/
_CODE_DIR    = _os.path.dirname(_CONFIG_DIR)                   # 4_Draw/code/
_LOCAL_ASSETS = _os.path.join(_CODE_DIR, 'Assets')
# Fall back to draw_all/code/Assets if no local copy exists
if _os.path.isdir(_LOCAL_ASSETS):
    _ASSETS_DIR = _LOCAL_ASSETS
else:
...
class MapConfig:
    def __init__(self, path='../output', input_name='', source_name=''):
        ...
        # 地图参数
        self.shp_path = _os.path.join(_ASSETS_DIR, "shp/GCCSA_line.shp")
        ...
        self.arrow_image_path = _os.path.join(_ASSETS_DIR, "north_arrow.png")

    def update_paths(self):
        """根据输入名称和来源名称更新文件路径"""
        base_name = f"{self.path}/{self.input_name}_{self.source_name}"

        self.basemap_geo_tif = _os.path.join(_ASSETS_DIR, "basemap.tif")
