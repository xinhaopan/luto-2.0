...
def is_zip_valid(path):
    ...
    if not os.path.exists(path):
        return False
    try:
        # 只需成功打开文件即可。构造函数会读取尾部的中央目录记录。
        # 这是最快的检查，因为它只读取文件末尾的一小部分元数据。
        with zipfile.ZipFile(path, 'r'):
            pass  # 如果能成功打开，就立即关闭并返回 True
...
# --- 主要逻辑开始 ---
os.chdir(r'output\20251009_Paper2_Results\Run_11_GHG_low_BIO_high_CUT_50')
# 定义归档文件路径
