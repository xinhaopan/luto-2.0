...
TASK_ROOT_DIR = '../Custom_runs/20250604_RES20_BETA_TEST'  # Do not include the trailing slash (/) in the end of the path
...
grid_search = {
    ...
    'RESFACTOR': [20],
    'SIM_YEARS': [list(range(2010, 2050, 10))],  # Years to run the model
    'WRITE_THREADS': [1],
    ...
    # Between 0 and 1, the higher the value, the more difficult it is to transition land use
    ...
    # between 0 and 1, if 1 will turn off biodiversity objective, if 0 will turn off profit objective
    'SOLVE_WEIGHT_BETA': [i for i in np.arange(0, 1.01, 0.01)],  # between 0 and 1, if 0 will turn off profit objective
...
if __name__ == '__main__':
    ...
    # create_task_runs(TASK_ROOT_DIR, grid_search_settings_df, mode='single',
    #                  n_workers=min(len(grid_search_param_df), 100))
