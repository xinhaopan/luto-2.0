...
def process_area_data(files, SAVE_DIR, lu_group_map):
    ...
    df_wide = am_dvar_area\
        .groupby(['region', 'Water_supply', 'Land-use', 'Type'])[['Year','Area (ha)']]\
        .apply(lambda x: x[['Year','Area (ha)']].values.tolist())\
        .reset_index()
    ...
    if non_ag_dvar_area.empty:
        df_wide = pd.DataFrame(columns=['region', 'name', 'data'])
    else:
...
def process_economics_data(files, SAVE_DIR):
    ...
    if not cost_transition_non_ag2ag_df.empty:
        cost_transition_non_ag2ag_df = cost_transition_non_ag2ag_df.dropna(subset=['Cost ($)'])
    ...
    cost_transition_non_ag2ag_df_non_all = cost_transition_non_ag2ag_df.query(
        ...
        ).copy() if not cost_transition_non_ag2ag_df.empty else pd.DataFrame()
    ...
    out_dict = {}
    _am_has_value = ('Value ($)' in revenue_am_df_non_all.columns or 'Value ($)' in cost_am_df_non_all.columns)
    if _am_has_value:
    ...
    out_dict = {}
    _non_ag_has_value = ('Value ($)' in revenue_non_ag_df.columns or 'Value ($)' in cost_non_ag_df.columns)
    if _non_ag_has_value:
    ...
    out_dict = {}
    if 'Value ($)' in revenue_am_df.columns or 'Value ($)' in cost_am_df.columns:
...
def process_production_data(files, SAVE_DIR, years):
    ...
    df_wide = quantity_am\
        .groupby(['region', 'Water_supply', 'Commodity', 'am'])[['Year','Production (t/KL)']]\
        .apply(lambda x: x[['Year','Production (t/KL)']].values.tolist())\
        .reset_index()

    df_wide.columns = ['region', 'water', 'commodity', 'name', 'data']
...
def process_ghg_data(files, SAVE_DIR, lu_group_map, years):
    ...
    # Am
    out_dict = {}
    if not GHG_ag_man_non_all.empty:
    ...
    out_dict = {}
    if not Non_ag_reduction_long.empty:
    ...
    Ag_man_sequestration_long = GHG_land\
        .query('Type == "Agricultural Management" and `Land-use` != "ALL"')\
        .reset_index(drop=True)
    Ag_man_sequestration_long['Value (t CO2e)'] = Ag_man_sequestration_long['Value (t CO2e)'] * -1  # Convert from negative to positive

    df_wide = Ag_man_sequestration_long\
        .groupby(['region', 'Water_supply', 'Land-use', 'Agricultural Management Type'])[['Year','Value (t CO2e)']]\
        .apply(lambda x: x[['Year', 'Value (t CO2e)']].values.tolist())\
        .reset_index()
    df_wide.columns = ['region', 'water', 'landuse', 'name', 'data']
    df_wide['type'] = 'column'
...
def process_water_data(files, SAVE_DIR):
    ...
    water_net_yield_watershed_non_all = water_net_yield_watershed\
        .query('`Water Supply` != "ALL" and Landuse	 != "ALL"')\
        .reset_index(drop=True)
    ...
    out_dict = {}
    _water_nonag_src = water_net_yield_NRM_region.query('Type == "Non-Agricultural Land-use"')
    if not _water_nonag_src.empty:
        water_nonag_AUS = _water_nonag_src\
        ...
        water_nonag = pd.concat([
            water_nonag_AUS,
            _water_nonag_src.rename(columns={'region_NRM': 'region'})
            ], ignore_index=True)
...
def process_biodiversity_data(files, SAVE_DIR):
    ...
    # ---------------- Overall quality - Ag ----------------
    bio_df_ag = bio_df.query('Type == "Agricultural land-use"').copy()
