...
from luto.tools.report.map_tools.helper import (
    get_map_meta, 
    get_map_fullname,
    get_scenario
)
...
def TIF2MAP(raw_data_dir:str):
    ...
    # Parallely create maps
    tasks = (
        delayed(create_maps)(row, model_run_scenario) 
        for _,row in tif_files_with_meta.iterrows()
    )
    for out in Parallel(n_jobs=settings.WRITE_THREADS, return_as='generator')(tasks):
        print(out)
